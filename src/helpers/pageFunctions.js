...
export function createCityElement(cityInfo) {
  ...
  const button = createElement('button', 'showForecast', 'Ver previsão');
  cityElement.appendChild(button);

  const daysForForecast = 7;

  button.addEventListener('click', async () => {
    const data = await fetch(`http://api.weatherapi.com/v1/forecast.json?lang=pt&key=${key}&q=${url}&days=${daysForForecast}`);
    const forecastData = await data.json();

    const forecastDays = forecastData.forecast.forecastday.map((day) => {
      return {
...
export async function handleSearch(event) {
  ...
  const cityweather = await Promise.all(cities
    .map(async (city) => getWeatherByCity(city.url)));
  cityweather.forEach((city) => {
    const showdebola = document.getElementById('cities');
    const bolashow = createCityElement(city);
    showdebola.appendChild(bolashow);
  });
}
